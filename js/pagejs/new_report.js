    ...
    if (com_content == "") {
        ...
        getajax_async(command_url, data, function (json) {
            if (json.result.errMsg == "success") {
                //dd_toast('点评成功！', 'success', 'success', 0);
                //点评完清空文本框内容
                $("#com_content").val('');
                //点评完直接显示点评内容
                $("#dianping").append('<p  style="color:#7f9bec">' + username + ':' + com_content + '</p>');
                var dianping_content = $("#dianping >p").html();
                //第一次点评需要刷新页面
                if (dianping_content == '') {
    ...
    reportInfo: function () {
        ...
        getajax_async(report_url, data, function (json) {
            if (json.result.errMsg == "success") {
                ...
                if (dianping != "") {
                    $("#dianping").css({ 'line-height': '0.3rem', 'padding': '0.2rem' });
                    var dianpings = dianping.split(',');
                    for (var i = 0; i < dianpings.length; i++) {
                        $("#dianping").append('<p style="color:#7f9bec">' + dianpings[i] + '</p>');
                    }
                }
                else {
                    $("#dianping").parent().empty();
                }
